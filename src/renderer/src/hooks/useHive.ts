...
const initialGodPrompt = (godName: string): string => [
  ...
  '4. Skim COMMANDS.md (hive root) for the Claude Code commands you can use.',
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      const res = await window.cth.spawnPty({
        ...
        resume: true,
        hive: { id: GOD_ID, name: godName, provider: godProvider, cwd: config.harnessHome!, isGod: true, role: 'runs the floor' }
      });
