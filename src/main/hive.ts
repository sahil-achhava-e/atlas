...
import { statusWithOpenAsks } from '../shared/humanQA';
...
export interface AgentMeta {
  ...
  role?: string;
  capabilities?: string[];
  /** Marked by the human as a project's team leader: coordinates that project's
...
/** What `hive/environment.json` holds — the app's settings as the crew needs to
 *  understand them, and nothing that is a secret. */
export interface HiveEnvironment {
  /** Agents act without asking per tool call. */
  autoMode: boolean;
  /** The orchestrator may start ephemeral workers itself. */
  orchestratorMaySpawn: boolean;
  /** How many ephemeral workers may run at once. */
  maxConcurrentWorkers: number;
  /** What a new agent starts on unless told otherwise. */
  defaultModel?: string;
  defaultWorkerTokenCap?: number;
  /** The engine the orchestrator itself runs on. */
  godProvider?: string;
  godModel?: string;
  /** Repos registered as the work. */
  projects: string[];
  /** MCP servers every new agent is given, by id. */
  mcpEnabled: string[];
  ...
  /** Scheduled work the floor runs on its own. */
...
export class HiveManager {
  ...
  /**
   * Write `hive/environment.json` — what the app is CONFIGURED to be.
   *
   * The orchestrator could see the floor (registry, fleet, board, tasks) and
   * nothing about the settings that govern it: whether agents may act
   * unattended, what model a new hire starts on, which MCP servers they are
   * given, which databases exist and for which project, what skills are on the
   * machine, how many workers may run at once. Asked any of it he had to guess,
   * and he guessed — "no client installed", "nothing is running".
   *
   * A FILE, not the prompt. The prompt is cached per spawn and must stay free of
   * anything that changes (see the cache invariant above); this changes whenever
   * the human touches Settings. One static line points him here, and he reads it
   * when the answer matters — the same contract as fleet.json.
   *
   * NO SECRETS. Database URLs, API keys and MCP credentials never appear: what
   * a database IS (its label and its project) is what routing needs, and the
   * connection string is the agent's to fetch from the broker at run time.
   */
  writeEnvironment(env: HiveEnvironment): void {
    const root = this.root();
    if (!root) return;
    try { this.writeJson(join(root, 'environment.json'), { ts: Date.now(), ...env }); }
    catch (e) { console.error('[hive] environment.json:', e); }
  }
  ...
  private injectedPrompt(
    ...
    const spawnQueueLine = meta.isGod && this.orchestratorMaySpawn()
      ? `SPAWNING A WORKER: you can start an ephemeral worker yourself by writing ONE JSON file into ${inRoot('spawn-requests')}/<id>.json. Required: \`objective\` (what the worker must do) and \`cwd\` (the repo it runs in). Optional: \`name\`, \`command\`, \`provider\`, \`model\`, \`isolate\` (default true = its own git worktree), \`tokenCap\`, and \`slack\` ({channel, thread_ts}) to route its failures back to a thread. The harness polls that directory, spawns \`worker-<id>\`, and moves the request to \`spawn-requests/.done/\` on success or \`.failed/\` with a reason. This is the ONLY way you can spawn; a hire manifest under research/hires/ needs the human to confirm it in the UI, so it is not a route you can complete on your own. ASKING FOR A PERMANENT HIRE: when the floor needs a lasting role rather than a one-off worker, ASK THE HUMAN — never keep spawning ephemeral workers to paper over a missing teammate. Put the ask on the ASK ME board (status \"blocked\" + a humanQA entry on the card that needs it, or a direct message to the human if no card fits) and give ALL of: (1) ROLE — the job title in two or three words; (2) WHY — what keeps coming up that no current agent owns, with the evidence (cards, message ids, how many times this week); (3) PROJECT — the exact repo/folder it would work in; (4) WHY NOT AN EXISTING AGENT — name the closest fit on the roster and say why it does not cover this; (5) ENGINE + BUDGET — the provider/model you would give it and a token cap, with the cost implication stated plainly; (6) FIRST TASK — the concrete first card it would pick up. Without all six the human cannot answer, and an ask they cannot answer is noise. Reuse an existing agent first, as above — a worker is a fresh spend every time. CHOOSING ITS MODEL: leave \`model\` out and the worker starts on Sonnet, which is right for most work. Set \`model\` to 'claude-opus-4-8[1m]' when the job needs real judgement — a design call, a gnarly bug, work you cannot specify precisely — and to 'claude-haiku-4-5-20251001' for narrow mechanical work like triage, routing, formatting or a verification pass. The difference in cost between those three is large, so pick deliberately rather than reaching for the strongest every time.`
      : '';
    // One static line — the file behind it changes, this sentence does not, so
    // the prompt cache is untouched.
    const envLine = `HOW THIS FLOOR IS SET UP: ${inRoot('environment.json')} is the human's settings as they affect you — autonomy, the model and token cap a new agent starts on, how many workers may run at once, which MCP servers every agent is given, which databases exist and which project each belongs to, the skills THE WHOLE FLOOR has (every agent can invoke an untagged one wherever it is working, so you can name one in a dispatch and the agent will have it; a skill carrying a \`project\` belongs to that repo alone, so only an agent working there has it), whether semantic memory and the knowledge graph are on, and the scheduled missions. READ IT before you answer a question about what the floor can do, and re-read it rather than remembering — the human changes these while you are running. It never contains a secret: a database's connection string is not there, and you get one from the integration broker at run time. If something you need is off or missing, say which setting and where it lives (Settings → the tab) instead of guessing that it is impossible.`;
    ...
    const craftGodLine = meta.isGod
      ...
      : '';
    // WRITING A BRIEF. The human hires in Add Agent, which asks three things: a
    // one-line title, "What does it do?" and "When is it finished?". Asked in
    // chat for a new hire's brief, god used to answer with one generic
    // paragraph: no lead, no reporting line, nothing true about the repo. This
    // is the shape the good briefs on the floor already have. Static text.
    const briefLine = meta.isGod
      ? 'WRITING A BRIEF FOR A NEW HIRE: when the human asks you for one, write it for them in chat, ready to paste into Add Agent, in exactly three labelled parts that match the dialog. **Title** — one line, role and project ("Senior Frontend - Events"). **What does it do?** — short paragraphs, each opening with a CAPS lead-in, in this order: (1) WHO YOU ARE: "You are the <role> on <project>, in <folder> (<stack, port>)", and that they are experienced, so the brief spends its words on what is true HERE; (2) WHO YOU REPORT TO: name the project\'s LEAD from the live roster, say work arrives as a card assigned to them in tasks.json and results go back to that lead as a summary, that they do not message you or the human directly, and that when blocked they tell the lead and pick up something else (no lead on the project: they report to you, and say so); (3) THE STACK AND CONVENTIONS to reuse instead of reaching for new ones; (4) THINGS THAT ARE ALWAYS WRONG HERE, numbered, each with the right way; (5) BRANCHING, REVIEW AND RELEASE: the base branch, the review skill to run before committing, what they must never hand-edit; (6) ASK <LEAD> FIRST: new dependencies, auth or routing changes, scope beyond the card; (7) verify before reporting and quote what the check printed. **When is it finished?** — one paragraph: card to `in-review` when the PR is open, PR against the right base, the lead gets the PR id, one line on what it does, the check output and exactly what to click; they never merge, and the lead picks the reviewer. EVERY FACT COMES FROM THE PROJECT: read its README, CLAUDE.md, package.json, neighbouring code and the other briefs for that project before you write, and leave a section out rather than invent one. Do not write the "you work only in <project>" or "ask the human when" lines: the dialog adds both. After the brief, say in one line which engine and model you would give it and why.'
      : '';
  ...
  writeTasks(tasks: HiveTask[]): void {
    ...
    // Status is settled here, on the one path every writer goes through: the
    // orchestrator, a worker, the voice actions, a webhook and the board UI all
    // land in writeTasks. Two rules, in order — normalize the spelling, then
    // hold the card blocked while it still has an unanswered question for the
    // human. The second one is why a card can no longer carry an open ask off
    // the ASK ME board (which lists blocked cards only) and strand it where
    // nobody can answer or dismiss it.
    const merged = mergeTaskLedger(current?.tasks, tasks).map((t) => (
      t && typeof t === 'object' && !Array.isArray(t) && 'status' in (t as object)
        ? {
            ...(t as Record<string, unknown>),
            status: statusWithOpenAsks(
              normalizeStatus((t as { status?: unknown }).status),
              t as { humanQA?: unknown }
            )
          }
        : t));
  ...
  rosterContext(
    ...
    try {
      ...
      // WHO LEADS WHAT. The roster listed a lead as just another agent with a
      // longer role string, so routing to "someone who fits" picked whoever
      // looked closest to the task — usually an engineer, often nobody, and
      // the work came back to the orchestrator to do himself.
      const leads = new Set(Object.values(this.registry().agents ?? {})
        .filter((a) => a?.isLead).map((a) => a.id));
      ...
      const rows = shown.map((a) => {
        ...
        if (leads.has(a.id)) bits.push('TEAM LEAD — route this project through them');
...
- One question per message, and make it answerable: "ship it or hold?" beats "what
  ...
  message that sits.
- A question that offers a choice, to the human or to another agent, is never one
  paragraph. Open with ONE **bold** sentence saying what you need. Put each option on
  its own \`-\` bullet, saying what it **affects** (what changes, what it costs, what
  breaks or has to be redone). End with a **Recommended:** line naming the option you
...
\`\`\`json
{ "q": "the ask, in markdown", "by": "god", "askedAt": "<iso timestamp>" }
\`\`\`

\`by\` is YOUR agent id, and it matters: without it the board labels the ask with the CARD'S ASSIGNEE,
...
is worth the human's attention, rewrite it as one answerable question, and put it on the card. Nobody
else appends to \`humanQA\`, and nobody else asks the human directly.
...
- \`-\` bullets or \`1.\` numbering for every option or step, each saying what it **affects**
  (what changes, what it costs, what breaks or has to be redone);
- end with a **Recommended:** line naming the option you would pick and why, in one sentence.
  The human should never have to work out your view from the options;
