...
const api = {
  ...
  skillsAddLocal: (dir: string): Promise<{ ok: true; path: string; name: string } | { ok: false; error: string }> =>
    ipcRenderer.invoke('skills:addLocal', dir),
  ...
  // Database connections: the list lives in config (labels only), each URL in
  // the encrypted store. Write-only, like every other credential here.
