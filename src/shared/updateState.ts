...
export function installerUrl(version: string, platform: string, arch: string): string {
  ...
  const file = platform === 'darwin' ? `Atlas-${v}-mac-${arch}.dmg`
    : platform === 'win32' ? `Atlas-${v}-win-x64-setup.exe`
    : `Atlas-${v}-linux-x86_64.AppImage`;
  ...
  switch (status?.state) {
    ...
    case 'downloaded':
      return {
        ...
        detail: `Restart Atlas to finish updating from v${v}.`,
...
export function manualInstallSteps(platform: string): { os: string; steps: string[] } {
  if (platform === 'darwin') {
    return {
      os: 'macOS',
      steps: [
        'Open the .dmg and drag Atlas onto Applications. Choose Replace when asked.',
  ...
  if (platform === 'win32') {
    return {
      ...
      steps: [
        ...
        'Open Atlas again and pick the same project.'
      ]
    };
  }
  return {
