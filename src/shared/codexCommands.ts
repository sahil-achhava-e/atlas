...
export const CODEX_COMMAND_GROUPS: CmdGroup[] = [
  ...
  },
  {
    ...
    items: [
      { cmd: 'codex --dangerously-bypass-approvals-and-sandbox', kind: 'cli', desc: 'Skip ALL approval prompts AND drop the OS sandbox (full filesystem access). Atlas no longer uses this for auto mode; it keeps the sandbox and adds the hive agent folder via --add-dir.' },
      { cmd: 'codex -a never -s workspace-write', kind: 'cli', desc: 'Never prompt for approval (-a never) but keep the sandbox scoped to the project workspace (-s workspace-write). What Atlas uses for auto mode; the hive agent folder is added with --add-dir <dir>.' },
  ...
  },
  {
    ...
    items: [
      ...
      { cmd: 'CODEX_NON_INTERACTIVE=1 codex', kind: 'cli', desc: 'Suppress all interactive installer / first-run prompts. Set automatically by Atlas in auto mode.' }
    ]
  },
  {
    title: 'CONFIG',
